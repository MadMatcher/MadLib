...
class TestJaccardFeature:
    ...
    def test_init(self, tokenizer):
        ...
        jaccard_feature = JaccardFeature('a_attr', 'a_attr', tokenizer)
        assert jaccard_feature.a_attr == 'a_attr'
        assert jaccard_feature.b_attr == 'a_attr'
        assert jaccard_feature._tokenizer == tokenizer
        assert jaccard_feature._a_toks_col == tokenizer.out_col_name('a_attr')
        assert jaccard_feature._b_toks_col == tokenizer.out_col_name('a_attr')
    ...
    def test_str(self, tokenizer):
        ...
        jaccard_feature = JaccardFeature('a_attr', 'a_attr', tokenizer)
        assert str(jaccard_feature) == f'jaccard({tokenizer.out_col_name("a_attr")}, {tokenizer.out_col_name("a_attr")})'
...
class TestOverlapCoeffFeature:
    def test_init(self, tokenizer):
        ...
        overlap_coeff_feature = OverlapCoeffFeature('a_attr', 'a_attr', tokenizer)
        assert overlap_coeff_feature.a_attr == 'a_attr'
        assert overlap_coeff_feature.b_attr == 'a_attr'
        assert overlap_coeff_feature._tokenizer == tokenizer
        assert overlap_coeff_feature._a_toks_col == tokenizer.out_col_name('a_attr')
        assert overlap_coeff_feature._b_toks_col == tokenizer.out_col_name('a_attr')
    ...
    def test_str(self, tokenizer):
        ...
        overlap_coeff_feature = OverlapCoeffFeature('a_attr', 'a_attr', tokenizer)
        assert str(overlap_coeff_feature) == f'overlap_coeff({tokenizer.out_col_name("a_attr")}, {tokenizer.out_col_name("a_attr")})'
...
class TestCosineFeature:
    def test_init(self, tokenizer):
        ...
        cosine_feature = CosineFeature('a_attr', 'a_attr', tokenizer)
        assert cosine_feature.a_attr == 'a_attr'
        assert cosine_feature.b_attr == 'a_attr'
        assert cosine_feature._tokenizer == tokenizer
        assert cosine_feature._a_toks_col == tokenizer.out_col_name('a_attr')
        assert cosine_feature._b_toks_col == tokenizer.out_col_name('a_attr')
    ...
    def test_str(self, tokenizer):
        ...
        cosine_feature = CosineFeature('a_attr', 'a_attr', tokenizer)
        assert str(cosine_feature) == f'cosine({tokenizer.out_col_name("a_attr")}, {tokenizer.out_col_name("a_attr")})' 
...
class TestMongeElkanFeature:
    def test_init(self, tokenizer):
        ...
        monge_elkan_feature = MongeElkanFeature('a_attr', 'a_attr', tokenizer)
        assert monge_elkan_feature.a_attr == 'a_attr'
        assert monge_elkan_feature.b_attr == 'a_attr'
        assert monge_elkan_feature._tokenizer == tokenizer
        assert monge_elkan_feature._a_toks_col == tokenizer.out_col_name('a_attr')
        assert monge_elkan_feature._b_toks_col == tokenizer.out_col_name('a_attr')
    ...
    def test_str(self, tokenizer):
        ...
        monge_elkan_feature = MongeElkanFeature('a_attr', 'a_attr', tokenizer)
        assert str(monge_elkan_feature) == f'monge_elkan_jw({"a_attr"}, {"a_attr"})'
